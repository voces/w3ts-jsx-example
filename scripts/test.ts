...
if (gameExecutable == null) {
	console.error(
		new Error(
			"Environmental variable 'WAR3_PATH' is unset. You should set it to the path of your WarCraft 3 executable.",
		),
	);
	process.exit(1);
}
...
execFile(
	...
	(err: any) => {
		if (err?.code === "ENOENT")
			logger.error(
				`No such file or directory "${gameExecutable}". Make sure environmental variable 'WAR3_PATH' is configured properly.`,
			);
	},
);
