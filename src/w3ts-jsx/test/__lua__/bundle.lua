...
["src.w3ts-jsx.test.build.test"] = function() local ____exports = {}
local tstl = require("node_modules.typescript-to-lua.dist.index")
local ____fs = require("src.fs")
local readFileSync = ____fs.readFileSync
local globals = _G
...
it:skip(
    "works",
    function()
        local result = tstl:transpileProject("src/w3ts-jsx/test/test.tsconfig.json")
        expect(nil, result.diagnostics):toHaveLength(0)
        local lua = readFileSync(nil, "src/test/__lua__/bundle.lua", "utf-8")
        expect(nil, lua):toMatchSnapshot()
        expect(
            nil,
            ((string.find(lua, "createTextElement", nil, true) or 0) - 1) >= 0
        ):toBeTruthy()
    end
)
return ____exports
end,
["src.basic-pragma.adapter"] = function() local ____exports = {}
...
____exports.setAdapter = function(____, adapter)
...
["src.w3ts-jsx.actualTypes"] = function() local ____exports = {}
return ____exports
end,
["src.w3ts-jsx.adapter"] = function() local ____exports = {}
local frameDefaults = {name = "", priority = 0, isSimple = true, typeName = nil, inherits = "", alpha = 255, enabled = true, font = {fileName = "", height = 16, flags = 0}, level = 0, maxLength = 9999, minMaxValue = {min = -999999999, max = 999999999}, model = {modelFile = "", cameraIndex = 0}, scale = 1, spriteAnimate = {primaryProp = 0, flags = 0}, stepSize = 0, text = "", textAlignment = {vert = TEXT_JUSTIFY_TOP, horz = TEXT_JUSTIFY_LEFT}, textColor = 16777215, texture = {texFile = "", flag = 0, blend = false}, tooltip = nil, value = 0, vertexColor = 16777215, visible = true, position = nil, absPosition = nil, size = {width = 0.1, height = 0.1}, children = nil}
local context = 0
local absurd
absurd = function(____, value)
    error(
...
local setProp
setProp = function(____, frame, prop, value)
    ...
    local ____switch4 = prop
    if ____switch4 == "text" then
        goto ____switch4_case_0
    elseif ____switch4 == "maxLength" then
        goto ____switch4_case_1
    elseif ____switch4 == "textColor" then
        goto ____switch4_case_2
    elseif ____switch4 == "texture" then
        goto ____switch4_case_3
    elseif ____switch4 == "model" then
        goto ____switch4_case_4
    elseif ____switch4 == "alpha" then
        goto ____switch4_case_5
    elseif ____switch4 == "level" then
        goto ____switch4_case_6
    elseif ____switch4 == "visible" then
        goto ____switch4_case_7
    elseif ____switch4 == "enabled" then
        goto ____switch4_case_8
    elseif ____switch4 == "vertexColor" then
        goto ____switch4_case_9
    elseif ____switch4 == "value" then
        goto ____switch4_case_10
    elseif ____switch4 == "size" then
        goto ____switch4_case_11
    elseif ____switch4 == "stepSize" then
        goto ____switch4_case_12
    elseif ____switch4 == "tooltip" then
        goto ____switch4_case_13
    elseif ____switch4 == "font" then
        goto ____switch4_case_14
    elseif ____switch4 == "minMaxValue" then
        goto ____switch4_case_15
    elseif ____switch4 == "scale" then
        goto ____switch4_case_16
    elseif ____switch4 == "spriteAnimate" then
        goto ____switch4_case_17
    elseif ____switch4 == "textAlignment" then
        goto ____switch4_case_18
    elseif ____switch4 == "position" then
        goto ____switch4_case_19
    elseif ____switch4 == "absPosition" then
        goto ____switch4_case_20
    elseif ____switch4 == "name" then
        goto ____switch4_case_21
    elseif ____switch4 == "priority" then
        goto ____switch4_case_22
    elseif ____switch4 == "isSimple" then
        goto ____switch4_case_23
    elseif ____switch4 == "typeName" then
        goto ____switch4_case_24
    elseif ____switch4 == "inherits" then
        goto ____switch4_case_25
    elseif ____switch4 == "children" then
        goto ____switch4_case_26
    end
            ...
            if val ~= nil then
                for ____, position in ipairs(val) do
                    ...
                    elseif position == "parent" then
                        ...
                        BlzFrameSetPoint(frame, position.point, position.relative, position.relativePoint, position.x or 0, position.y or 0)
                    end
                end
            end
            goto ____switch4_end
...
["src.basic-pragma.common"] = function() local ____exports = {}
____exports.TEXT_ELEMENT = "TEXT ELEMENT"
return ____exports
end,
["src.basic-pragma.reconciler"] = function() require("lualib_bundle");
local ____exports = {}
local ____adapter = require("src.basic-pragma.adapter")
local adapter = ____adapter.adapter
local ____common = require("src.basic-pragma.common")
local TEXT_ELEMENT = ____common.TEXT_ELEMENT
local cleanupFrames, reconcileChildren, instantiate, createPublicInstance, updateInstance
function ____exports.reconcile(self, parentFrame, instance, vnode)
    do
        local ____try, err, ____returnValue = pcall(
            function()
                if not instance then
                    return true, instantiate(nil, vnode, parentFrame)
                elseif not vnode then
                    cleanupFrames(nil, instance)
                    return true, nil
                elseif instance.vnode.type ~= vnode.type then
                    local newInstance = instantiate(nil, vnode, parentFrame)
                    cleanupFrames(nil, instance)
                    ...
                    if type(vnode.type) == "string" then
                        ...
                        instance.childInstances = reconcileChildren(nil, instance, vnode)
                    ...
                    elseif instanceOfSameType.publicInstance then
                        ...
                        local childInstance = ____exports.reconcile(nil, parentFrame, oldChildInstance, childElement)
...
function reconcileChildren(self, instance, vnode)
    ...
    do
        ...
        while i < count do
            ...
            local newChildInstance = ____exports.reconcile(nil, hostFrame, childInstance, childElement)
...
function instantiate(self, vnode, parentFrame)
    ...
    if type(____type) == "string" then
        ...
        local childInstances = __TS__ArrayMap(
            childElements,
            function(____, child) return instantiate(nil, child, frame) end
        )
        local instance = {hostFrame = frame, vnode = vnode, childInstances = childInstances, childInstance = nil}
        return instance
    else
        local instance = {}
        local publicInstance = createPublicInstance(nil, vnode, instance)
        ____exports.hooks:beforeRender(publicInstance)
        local childElement = publicInstance:render(props)
        local childInstance = (childElement and instantiate(nil, childElement, parentFrame)) or nil
...
function createPublicInstance(self, vnode, internalInstance)
    local ComponentType = vnode.type
    local props = vnode.props
    local constructor
    if type(ComponentType) == "string" then
        error("Tried createPublicInstance() with string", 0)
    elseif type(ComponentType) ~= "function" then
...
function updateInstance(self, internalInstance)
    local parentDom = adapter:getParent(internalInstance.hostFrame)
    local vnode = internalInstance.vnode
    if parentDom then
        ____exports.reconcile(nil, parentDom, internalInstance, vnode)
    else
        error("Tried to reconcile instance with no dom.parentDom", 0)
    end
end
____exports.hooks = {
    beforeRender = function(____, instance)
    end
}
local rootInstance = nil
function ____exports.render(self, vnode, container)
    print("render start")
    local prevInstance = rootInstance
    local nextInstance = ____exports.reconcile(nil, container, prevInstance, vnode)
...
function ClassComponent.prototype.setState(self, partialState)
    ...
    if instance then
        updateInstance(nil, instance)
    end
end
return ____exports
end,
["src.basic-pragma.Component"] = function() local ____exports = {}
return ____exports
end,
["src.basic-pragma.element"] = function() require("lualib_bundle");
local ____exports = {}
local ____common = require("src.basic-pragma.common")
local TEXT_ELEMENT = ____common.TEXT_ELEMENT
local createTextElement
function ____exports.createElement(self, ____type, config, children)
    ...
    local flattenedChildren = __TS__ArrayMap(
        __TS__ArrayFilter(
            ((children and (#children > 0)) and children) or ({}),
...
["src.w3ts-jsx.index"] = function() local ____exports = {}
local ____adapter = require("src.w3ts-jsx.adapter")
local adapter = ____adapter.adapter
local ____adapter = require("src.basic-pragma.adapter")
local setAdapter = ____adapter.setAdapter
setAdapter(nil, adapter)
do
    local ____element = require("src.basic-pragma.element")
    local createElement = ____element.createElement
    ____exports.createElement = createElement
end
do
    local ____reconciler = require("src.basic-pragma.reconciler")
    local render = ____reconciler.render
    ____exports.render = render
end
return ____exports
end,
["src.w3ts-jsx.test.sample"] = function() local ____exports = {}
local React = require("src.w3ts-jsx.index")
local App
...
return require("src.w3ts-jsx.test.sample")
