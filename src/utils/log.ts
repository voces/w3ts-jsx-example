...
const isArray = (v: any): boolean => {
	...
	return (
		Object.keys(v).every(
			(v, index) => S2I(v) === index + 1 || S2I(v) === index,
		) &&
		(v[0] != null || v[1] != null)
	);
};
...
export const termToString = (v: any, color = true, level = 0): string => {
	...
	switch (type) {
		...
		case "unit":
			return `Unit ${termToString(
				{
					id: GetHandleId(v),
					name: GetUnitName(v),
					owner: colorizedName(GetOwningPlayer(v)),
				},
				color,
				level + 1,
			)}`;
		case "force": {
			...
			return `Force ${termToString(arr, color, level + 1)}`;
		}
		default: {
			let handleId = -1;
			try {
